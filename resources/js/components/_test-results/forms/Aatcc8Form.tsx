...
type Aatcc8Section = {
  [key: string]: Aatcc8Field | any;
};

type Props = {
  testId: number;
  aatcc8Section: Aatcc8Section;
};

const Aatcc8Form: React.FC<Props> = ({ testId, aatcc8Section }) => {
  const safeSection = aatcc8Section || {};
  ...
  return (
    <Card className="border-0 shadow-sm rounded-4">
      <Card.Body className="p-4">
        <h5 className="mb-3">AATCC 8 – Solidez del color al frote</h5>
        ...
        <Form onSubmit={handleSubmit}>
          <Row className="g-3">
            {fieldEntries.map(([key, field]) => {
              ...
              return (
                <Col md={6} key={key}>
                  <Form.Group controlId={`aatcc8-${key}`}>
          ...
          <p className="text-muted small">
            Captura o adjunta fotografías relacionadas con la prueba AATCC 8.
          </p>

          <CameraCapture
            inputId="aatcc8-camera"
            multiple={true}
          ...
          <div className="d-flex justify-content-end mt-4 gap-2">
            ...
            <Button
              ...
              disabled={processing}
            >
              {processing ? 'Guardando...' : 'Guardar AATCC 8'}
            </Button>
          </div>
        </Form>
      </Card.Body>
