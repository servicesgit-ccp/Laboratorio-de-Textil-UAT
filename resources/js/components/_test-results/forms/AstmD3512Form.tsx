...
type AstmD3512Section = {
  [key: string]: AstmD3512Field | any;
};

type Props = {
  testId: number;
  astmSection: AstmD3512Section;
};

const AstmD3512Form: React.FC<Props> = ({ testId, astmSection }) => {
  ...
  fieldEntries.forEach(([key, field]) => {
    const f = field as AstmD3512Field;
  ...
  return (
    <Card className="border-0 shadow-sm rounded-4">
      <Card.Body className="p-4">
        <h5 className="mb-3">ASTM D3512 – Pilling de la tela</h5>
        ...
        <Form onSubmit={handleSubmit}>
          <Row className="g-3">
            {fieldEntries.map(([key, field]) => {
              ...
              return (
                <Col md={6} key={key}>
                  <Form.Group controlId={`astmd3512-${key}`}>
          ...
          <div className="d-flex justify-content-end mt-4 gap-2">
            ...
            <Button
              ...
              disabled={processing}
            >
              {processing ? 'Guardando...' : 'Guardar ASTM D3512'}
            </Button>
          </div>
        </Form>
      </Card.Body>
