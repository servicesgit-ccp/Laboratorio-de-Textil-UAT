...
type AATCC135Section = {
  [key: string]: AATCC135Field | any;
};

type Props = {
  testId: number;
  aatccSection: AATCC135Section;
};

const AATCC135Form: React.FC<Props> = ({ testId, aatccSection }) => {
  ...
  return (
    <Card className="border-0 shadow-sm rounded-4">
      <Card.Body className="p-4">
        <h5 className="mb-3">AATCC 135 – Estabilidad Dimensional</h5>
        ...
        <Form onSubmit={handleSubmit}>
          <Row className="g-3">
            {fieldEntries.map(([key, field]) => {
              ...
              return (
                <Col md={6} key={key}>
                  <Form.Group controlId={`aatcc135-${key}`}>
          ...
          <div className="d-flex justify-content-end mt-4 gap-2">
            ...
            <Button
              ...
              disabled={processing}
            >
              {processing ? 'Guardando...' : 'Guardar AATCC 135'}
            </Button>
          </div>
        </Form>
      </Card.Body>
